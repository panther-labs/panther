...
func TestNewCounterAverage(t *testing.T) {
	buf := bytes.NewBuffer([]byte{})
	cm := NewCWEmbeddedMetrics(buf)
	// Stubbing the time function
	cm.timeFunc = func() int64 {
	...
	t.Run("counter average without dimensions", func(t *testing.T) {
		buf.Reset()
		counter := cm.NewCounterAvg("test", UnitBytes)
		counter.Add(10)
		assert.NoError(t, cm.Sync())
		// nolint: lll
		assert.Equal(t, `{"test":10,"_aws":{"CloudWatchMetrics":[{"Namespace":"Panther","Dimensions":[[]],"Metrics":[{"Name":"test","Unit":"Bytes"}]}],"Timestamp":1000}}`+"\n", buf.String())
	...
	t.Run("counter average multiple counters for same dimensions", func(t *testing.T) {
		buf.Reset()
		counter := cm.NewCounterAvg("test", UnitCount)

		counter.With("dimension1", "value1", "dimension2", "value2").Add(0)
		counter.With("dimension1", "value1", "dimension2", "value2").Add(10)
		assert.NoError(t, cm.Sync())
		// nolint: lll
		assert.Equal(t, `{"test":5,"dimension1":"value1","dimension2":"value2","_aws":{"CloudWatchMetrics":[{"Namespace":"Panther","Dimensions":[["dimension1","dimension2"]],"Metrics":[{"Name":"test","Unit":"Count"}]}],"Timestamp":1000}}`+"\n", buf.String())
	})

	t.Run("counter average multiple dimension values", func(t *testing.T) {
		buf.Reset()
		counter := cm.NewCounter("test", UnitCount)
