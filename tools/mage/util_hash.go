...
func sourceHashes() (map[string][16]byte, error) {
	// Similar to .gitignore, but not exactly the same.
	// These are top-level files/directories not affected by formatting or autogenerated files.
	...
	for _, info := range entries {
		if _, exists := ignoreSet[info.Name()]; !exists {
			hashPaths = append(hashPaths, info.Name())
		}
	}

	logger.Debugf("computing sourceHashes: %s", strings.Join(hashPaths, ", "))
	return fileHashMap(hashPaths...)
}
