...
describe('BulkUploader', () => {
  ...
  it('allows dropping a file', async () => {
    const mocks = [mockUploadPolicies({ {"data":{"uploadPolicies":{"totalPolicies":113,"modifiedPolicies":0,"newPolicies":0,"totalRules":110,"modifiedRules":0,"newRules":0,"totalGlobals":4,"newGlobals":0,"modifiedGlobals":0,"__typename":"UploadPoliciesResponse"}}} })];
    const file = new File([JSON.stringify({ ping: true })], 'bulkfile.zip', {
      type: 'zip',
    });

    const { container, getByTestId, getByText } = render(<BulkUploader />, { mocks });
