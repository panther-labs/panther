...
var pantherLogRegistry = map[string]PantherLogFactory{
	"default": func(typ string, tm time.Time, fields ...PantherField) interface{} {
...
//
// These are used in cases where a group of loggers using the prefix define
// domain specific PantherFieldKind enum values and need to handle them for correct JSON output.
//
// All unregistered prefixes will use the `default` factory which wraps `NewPantherLog` function
//
// Currently only `awslogs` require this mechanism but other log types might benefit in the future
//
// *WARNING*: The function is *not* thread safe and it's *only* meant to be called in an `init()` block
func RegisterPantherLogPrefix(name string, factory PantherLogFactory) {
	...
	// TODO: Add reflect based test with zero values to verify that all fields are
	// properly prefixed with `p_` and panic otherwise
